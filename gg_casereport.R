...
# value = c(1,1,2,3)
class = c("DPP-4","DPP-4","SU","SGLT2")
start_date = c("2012-02-20", "2012-10-21", "2014-08-10", "2015-12-27")
end_date   = c("2012-10-20", "2015-12-20", "2018-06-20", "2018-06-20")
df = data.frame(drug_name, class, start_date, end_date)
# df = data.frame(drug_name, start_date, end_date)
df

write.csv(df, file="drug_dataset.csv", row.names = F)
df = read.csv("drug_dataset.csv")
df

df2 = tidyr::gather(df, key="category", value = "date", start_date, end_date, -class)
